...
5. Partition candidate features into nameable semantic families after entity and grain are known, and record the evidence for each family. Same entity and grain make features eligible for comparison; they do not justify merging them.
6. Form Feature Group candidates from one semantic family at a time. Merge families only with explicit evidence that they share a business concept and a compatible lifecycle; otherwise keep them split as provisional candidates.
7. Separate targets from published features. Classify base, aggregated, derived, and request-time features and record their dependencies.
8. Design point-in-time behavior using both event validity and, when arrival can be delayed, availability time. Treat unresolved temporal semantics as a publication blocker.
9. Produce the requested deliverables using [references/deliverables.md](references/deliverables.md).
...
Treat shared entity, key, source, or refresh cadence as necessary compatibility signals, not as evidence of one semantic concept. For example, account profile/lifecycle fields and recent transaction behavior can share a customer-time grain while remaining separate Feature Groups. A proposed merge across such families requires an explicit, SQL- or requirement-backed reason; absent that reason, split them and mark unknown operational facts as open decisions.
